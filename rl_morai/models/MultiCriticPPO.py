...
class MultiCriticPPOAgent:
    ...
    def create_critic(self, env_id):
        self.critics[env_id] = PPOCritic(258).to(self.device)
        self.optimizers[env_id] = optim.Adam(self.critics[env_id].parameters(), lr=1e-4)

    def transfer_critic(self, from_env_id, to_env_id):
        self.critics[to_env_id] = PPOCritic(258).to(self.device)
        self.critics[to_env_id].load_state_dict(self.critics[from_env_id].state_dict())
        self.optimizers[to_env_id] = optim.Adam(self.critics[to_env_id].parameters(), lr=1e-4)
    ...
    def train(self, env_id):
        ...
        for next_state in next_states:
            if next_state is not None:
                with torch.no_grad():
                    ...
                    next_value = self.critics[env_id](feature).cpu().numpy()[0][0]
        ...
        for epoch in range(self.ppo_epochs):
            ...
            for start in range(0, len(states), self.mini_batch_size):
                ...
                new_values = self.critics[env_id](mb_states)
                ...
                if self.log_enabled:
                    self.writer.add_scalar(f"Critic/Loss_{env_id}", critic_loss.item(), self.update_count)
                ...
                self.optimizers[env_id].zero_grad()
                critic_total_loss.backward()
                torch.nn.utils.clip_grad_norm_(self.critics[env_id].parameters(), self.max_grad_norm)
                self.optimizers[env_id].step()
    ...
    def log_initial_reward(self, episode, reward, max_initial_episode=20):
        if self.log_enabled and episode <= max_initial_episode:
            self.writer.add_scalar("Episode/Initial_Reward", reward, episode)
    ...
    def load_critic(self, env_id, path):
        self.critics[env_id] = PPOCritic(258).to(self.device)
        self.critics[env_id].load_state_dict(torch.load(os.path.join(path, f"PPO_critic_{env_id}.pt")))
        self.optimizers[env_id] = optim.Adam(self.critics[env_id].parameters(), lr=1e-4)

    def save_model(self, save_path, env_id):
        ...
        torch.save(self.critics[env_id].state_dict(), os.path.join(save_path, f"PPO_critic_{env_id}.pt"))
        print(f"모델 저장 완료: {save_path}")

    def load_model(self, load_path, env_id):
