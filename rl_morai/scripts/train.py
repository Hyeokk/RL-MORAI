...
def main():
    ...
    env.set_episode_over_fn(TerminatedFns.cte_done(sensor, max_cte=0.3))
    ...
    try:
        for episode in range(num_episodes):
            ...
            # 모델 저장 - 지정된 절대 경로에 저장
            if (episode + 1) % 100 == 0:
                actor_path = os.path.join(SAVE_DIR, "ddpg_actor.pt")
                critic_path = os.path.join(SAVE_DIR, "ddpg_critic.pt")
                
                try:
                    torch.save(agent.actor.state_dict(), actor_path)
                    torch.save(agent.critic.state_dict(), critic_path)
                    print(f"[INFO] Save Model: {actor_path}, {critic_path}")
        ...
        # 마지막 모델 저장 시도
        try:
            final_actor_path = os.path.join(SAVE_DIR, "ddpg_actor.pt")
            final_critic_path = os.path.join(SAVE_DIR, "ddpg_critic.pt")
            torch.save(agent.actor.state_dict(), final_actor_path)
            torch.save(agent.critic.state_dict(), final_critic_path)
            print(f"[INFO] Save Model: {final_actor_path}, {final_critic_path}")
        except Exception as e:
            print(f"[ERROR] Fail Model: {e}")
